...
io.on('connection', (socket) => {
    ...
    // ⚡ Handler para atualizações de vida de entidades (qualquer player pode enviar)
    socket.on('entity-health-update', (data) => {
        ...
        // Retransmitir para todos os jogadores na sala (incluindo o remetente)
        io.to(player.roomId).emit('entity-health-update', {
            ...data,
            attackerId: socket.id // Identificar quem atacou
        });
        heartbeats.set(socket.id, Date.now());
    });
    
    // ⚡ Handler para morte de entidades (qualquer player pode enviar)
    socket.on('entity-died', (data) => {
        ...
        // Retransmitir para todos os jogadores na sala
        io.to(player.roomId).emit('entity-died', {
            ...data,
            killerId: socket.id // Identificar quem matou
        });
        heartbeats.set(socket.id, Date.now());
    });
    
    // ⚡ Handler para recursos colhidos (qualquer player pode enviar)
    socket.on('resource-harvested', (data) => {
        ...
        // Retransmitir para todos os jogadores na sala
        io.to(player.roomId).emit('resource-harvested', {
            ...data,
            harvesterId: socket.id // Identificar quem colheu
        });
        heartbeats.set(socket.id, Date.now());
    });
